...
export class MahoragaFighter extends Fighter {
   ...
   * When adapted to Gojo's Unlimited Void Domain (gojoDomainAdapted), aim rotation is explicitly enabled.
   */
  canAim() {
    ...
    if (this.gojoDomainAdapted || this.gojoAdapted?.domain || this.domainImmunity) {
      return true;
    }

    // When NOT adapted and inside enemy Gojo's Unlimited Void, disable auto-aim
    ...
    if (bounced && !this.isPulledOrDragged()) {
      ...
      if (canReboundDash) {
        ...
        if (target && !target.isDead && target.hp > 0) {
          ...
          const dashSpeed = Math.max(currentSpeed * 1.5, (this.speed || 6.5) * 2.2, 16.0);
          this.vx = (dx / dist) * dashSpeed;
          this.vy = (dy / dist) * dashSpeed;
          this.isWallReboundDashing = true;
          ...
          spawnSparks(this.x, this.y, 14, 'silver', '#FFFFFF');
          ...
          if (!this._lastWallBounceDashTime || (now - this._lastWallBounceDashTime > 1200)) {
  ...
  shoot(ownerIndex) {
    ...
    const opponent = this._findClosestEnemy();
    if (opponent && !opponent.isDead) {
      this.aim(opponent);
    }
    const distToOpponent = opponent ? Math.hypot(this.x - opponent.x, this.y - opponent.y) : Infinity;
    const swordRange = CONFIG.mahoraga?.swordRange ?? 110;
    const meleeDist = this.r + (opponent ? opponent.r : 25) + swordRange;
  ...
  update(opponent, ownerIndex, arena) {
    ...
    if (this.adaptationDashTimer > 0 && !isCaughtInUltimateBeam && !this.isPulledOrDragged()) {
      ...
      const dashDx = this.dashToX - this.dashFromX;
      const dashDy = this.dashToY - this.dashFromY;
      const dashDist = Math.hypot(dashDx, dashDy) || 1;
      const moveSpeed = this.speed || 6.5;
      const dashSpeed = Math.max(moveSpeed, dashDist / (maxDash || 10));
      this.vx = (dashDx / dashDist) * dashSpeed;
      this.vy = (dashDy / dashDist) * dashSpeed;
      ...
      if (this.adaptationDashTimer === 0) {
        ...
        // Apply stationary physics on arrival frame so Mahoraga halts cleanly in place
        this.applyMovementPhysics(0);
      }
      return;
    }
    ...
    if (this.postDashPauseTimer > 0) {
      this.postDashPauseTimer--;
      this.vx = 0;
      this.vy = 0;
      if (this.postDashPauseTimer === 0) {
        // Pause finished: resume natural arena movement toward the opponent
    ...
    if (this.isThrowing) {
      ...
      if (isInHitReaction) {
        ...
        this.throwBarrageShotsLeft = 0;
        this.throwBarrageTimer = 0;
        this.throwCooldown = Math.max(this.throwCooldown || 0, 180);
        return;
      }
    ...
    if (this.isBlitzActive) {
      ...
      if (this.isWallSlamBlitz && target) {
        ...
        // Natural movement during blitz: Mahoraga never stops mid-air, moving naturally according to arena rules
        this.applyMovementPhysics();
        if (arena) this.resolveWallBounce(arena, target);
      }
      ...
      if (this.blitzTimer >= blitzInterval) {
        ...
        if (hitIndex < totalHits) {
          ...
          // Restore natural movement momentum upon finishing the blitz
          const naturalSpeed = this.speed || 6.5;
          this.vx = Math.cos(kbAngle) * naturalSpeed;
          this.vy = Math.sin(kbAngle) * naturalSpeed;
    ...
    if (target && (!target.isDead || target.isRevivingFromContract || target.isShatterReviving)) {
      ...
      const canActSkills = !isInHitReaction && !isTargetReforming && !this.isShouting && !this.isCleaving && !this.isThrowing && !this.isWallSlamActive && !this.isInfinityBlitz && !this.isPulledOrDragged();
    ...
    if (arena && (this.postDashPauseTimer || 0) <= 0) {
