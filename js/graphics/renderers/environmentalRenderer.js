...
let currentCjVibeOpacity = 0;

let _cachedSunsetDark = null;
let _cachedSunsetLight = null;
let _cachedSunVigDark = null;
let _cachedSunVigLight = null;
let _cachedRayGrad = null;
let _cachedAtmosphereW = 0;
let _cachedAtmosphereH = 0;

function _initAtmosphereGradients(ctx, cw, ch) {
  if (_cachedAtmosphereW !== cw || _cachedAtmosphereH !== ch || !_cachedSunsetDark) {
    _cachedAtmosphereW = cw;
    _cachedAtmosphereH = ch;

    _cachedSunsetDark = ctx.createLinearGradient(0, 0, 0, ch);
    _cachedSunsetDark.addColorStop(0.0, 'rgba(245, 158, 11, 0.10)');
    _cachedSunsetDark.addColorStop(0.45, 'rgba(217, 119, 6, 0.08)');
    _cachedSunsetDark.addColorStop(1.0, 'rgba(180, 83, 9, 0.14)');

    _cachedSunsetLight = ctx.createLinearGradient(0, 0, 0, ch);
    _cachedSunsetLight.addColorStop(0.0, 'rgba(251, 191, 36, 0.13)');
    _cachedSunsetLight.addColorStop(0.45, 'rgba(249, 115, 22, 0.10)');
    _cachedSunsetLight.addColorStop(1.0, 'rgba(234, 88, 12, 0.16)');

    const sunX = cw * 0.75;
    const sunY = ch * 0.20;
    const maxR = Math.max(cw, ch) * 0.85;

    _cachedSunVigDark = ctx.createRadialGradient(sunX, sunY, 20, cw / 2, ch / 2, maxR);
    _cachedSunVigDark.addColorStop(0.0, 'rgba(254, 240, 138, 0.08)');
    _cachedSunVigDark.addColorStop(0.5, 'rgba(180, 83, 9, 0.05)');
    _cachedSunVigDark.addColorStop(1.0, 'rgba(120, 53, 15, 0.12)');

    _cachedSunVigLight = ctx.createRadialGradient(sunX, sunY, 20, cw / 2, ch / 2, maxR);
    _cachedSunVigLight.addColorStop(0.0, 'rgba(255, 237, 213, 0.12)');
    _cachedSunVigLight.addColorStop(0.5, 'rgba(251, 146, 60, 0.06)');
    _cachedSunVigLight.addColorStop(1.0, 'rgba(154, 52, 18, 0.14)');

    _cachedRayGrad = ctx.createLinearGradient(0, 0, cw, ch);
    _cachedRayGrad.addColorStop(0.0, 'rgba(255, 251, 235, 0.07)');
    _cachedRayGrad.addColorStop(0.3, 'rgba(251, 191, 36, 0.04)');
    _cachedRayGrad.addColorStop(0.7, 'rgba(249, 115, 22, 0.03)');
    _cachedRayGrad.addColorStop(1.0, 'rgba(0, 0, 0, 0)');
  }
}

/**
 * Draws the iconic GTA: San Andreas warm golden sunset / Los Santos heat haze atmosphere filter
 * whenever CJ is present in the arena.
 * Optimized with static cached gradients and zero per-frame memory allocation.
 */
export function drawCjSanAndreasAtmosphere() {
  ...
  if (!isBattleState) {
    currentCjVibeOpacity = 0;
    return;
  }

  // Check if CJ is in the match
  const hasCj = Boolean(
  ...
  const hasOverridingDomain = Boolean(
    state.fighters && state.fighters.some(f => f && f.domainActive)
  );

  let targetOpacity = (hasCj && !hasOverridingDomain) ? 1.0 : 0.0;

  if (targetOpacity > currentCjVibeOpacity) {
    currentCjVibeOpacity += (targetOpacity - currentCjVibeOpacity) * 0.08;
  } else {
    currentCjVibeOpacity += (targetOpacity - currentCjVibeOpacity) * 0.08;
  }

  if (currentCjVibeOpacity < 0.005) {
    currentCjVibeOpacity = 0;
    return;
  }

  const op = currentCjVibeOpacity;
  const isDark = (state.arenaTheme === 'dark');
  const cw = canvas.width;
  const ch = canvas.height;

  _initAtmosphereGradients(ctx, cw, ch);

  ctx.save();
  ctx.setTransform(1, 0, 0, 1, 0, 0);
  ctx.globalAlpha = op;

  // 1. Full-Screen GTA San Andreas Los Santos Golden Sunset Gradient
  ctx.fillStyle = isDark ? _cachedSunsetDark : _cachedSunsetLight;
  ctx.fillRect(0, 0, cw, ch);

  // 2. Full-Screen Los Santos Heatwave Radial Sun Vignette
  ctx.fillStyle = isDark ? _cachedSunVigDark : _cachedSunVigLight;
  ctx.fillRect(0, 0, cw, ch);

  // 3. Full-Screen Subtle Diagonal Golden Hour Sunrays
  ctx.fillStyle = _cachedRayGrad;
  ctx.fillRect(0, 0, cw, ch);
