...
function renderPreviewForDef(def, cacheKey) {
  ...
  const isUpright = def.type === 'tanjiro' || def.type === 'nezuko' || def.type === 'zenitsu' || def.type === 'inosuke' || def.type === 'denji' || def.type === 'power' || def.type === 'uryu' || def.type === 'ulquiorra' || def.type === 'reze' || def.type === 'makima' || def.type === 'toji' || def.type === 'nanami' || def.type === 'yuji' || def.type === 'yuta' || def.type === 'gojo' || def.type === 'sukuna' || def.type === 'saitama' || def.type === 'genos' || def.type === 'ichigo' || def.type === 'mahito' || def.type === 'megumi' || def.type === 'nobara' || def.type === 'escanor' || def.type === 'zeus' || def.category === 'Anime';
