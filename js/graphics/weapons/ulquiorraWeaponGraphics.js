...
let _ulquiorraWeaponImage = null;
let _ulquiorraWeaponImageLoading = false;

export function _getUlquiorraWeaponImage() {
  if (_ulquiorraWeaponImage && _ulquiorraWeaponImage.complete && _ulquiorraWeaponImage.naturalWidth > 0) {
    return _ulquiorraWeaponImage;
  }
  if (!_ulquiorraWeaponImageLoading && typeof Image !== 'undefined') {
    _ulquiorraWeaponImageLoading = true;
    const img = new Image();
    img.onload = () => {
...
export function drawUlquiorraMurcielago(ctx, x = 0, y = 0, angle = 0, r = 25, isSwinging = false, swingProgress = 0, opts = {}) {
  ...
  const img = _getUlquiorraWeaponImage();
  if (img && img.complete && img.naturalWidth > 0) {
    ctx.save();
    ctx.imageSmoothingEnabled = false; // Authentic nearest-neighbor pixel art scaling
    const bladeLen = opts.bladeLen || 94;
    const s = bladeLen / 489.0;
    ctx.scale(s, s);
    // Tsuba guard peak is at (211, 182) in the 798x313 image
    ctx.drawImage(img, -211, -182);
    ctx.restore();
    ctx.restore();
    return;
  }
