...
const MAX_FLAME_PARTICLES = 150; // Increased from 60 for visual richness
...
export class FlamethrowerParticleSystem {
  ...
  update(dt) {
    ...
    if (this.active) {
      // Spawn 5-8 particles per frame for thick flame
      const spawnCount = 5 + Math.floor(Math.random() * 4);
...
export function drawOrangeFlamethrowerGun(ctx, x, y, gunAngle, r) {
  ...
  ctx.shadowColor = cfg.flamethrower.pilotCore;
  ctx.shadowBlur = 6;
  ctx.fill();
