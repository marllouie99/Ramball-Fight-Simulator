...
export function drawRedSniperGun(ctx, x, y, gunAngle, r, recoil = 0, ammo = 4, maxAmmo = 4, reloadTimer = 0, isReloading = false, flashTimer = 0) {
  ...
  // The magazine drops down when reloading
  let magDropY = 0;
  if (isReloading) {
     ...
     if (progress < 0.2) {
       ...
       magDropY = 20; // stays dropped out of frame or off screen
     }
  }

  if (magDropY < 15) { // don't draw if it's fully dropped
    ctx.save();
    ctx.translate(0, magDropY * s);
    // Magazine body
    drawPoly([
      [14, 12],
      [22, 12],
    ...
    // Ammo Indicator lights on magazine
    const pipHeight = 8;
    for (let i = 0; i < maxAmmo; i++) {
      ...
      if (isFilled) {
        ctx.shadowColor = colors.glowRed;
        ctx.shadowBlur = 4;
      } else {
        ctx.shadowBlur = 0;
      }
      ctx.beginPath();
      ctx.arc(17 * s, pipY * s, 0.8 * s, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
    ctx.shadowBlur = 0;
  ...
  if (flashTimer > 0) {
    ...
    ctx.shadowBlur = 15 * flashProgress;
