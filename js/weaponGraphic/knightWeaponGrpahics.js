export function drawGrayShield(ctx, x, y, gunAngle, blockFlashTimer, dashState, r, dashGlowFade = 0) {
  ctx.save();
  ctx.translate(x, y);
  const shieldAngle = (dashState === 'charging' || dashState === 'dashing') ? gunAngle : (gunAngle - Math.PI / 2);
  ...
  if (dashGlowFade > 0) {
    ...
    // Add extra glow to the energy lines
...
export function drawGraySwordProjectile(ctx, x, y, angle, scale = 1.0) {
  ...
  // Add a glowing aura to the thrown sword
  ctx.shadowColor = 'rgba(255, 230, 50, 0.8)';
  ctx.shadowBlur = 15;
