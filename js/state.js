...
async function _loadFighterClassMap() {
  if (!_FIGHTER_CLASS_MAP) {
    const mod = await import('./customFighters.js');
...
export function createFighterInstance(def, fighterIndex) {
  ...
  if (!_FIGHTER_CLASS_MAP) {
    ...
    fighter = new Fighter(def);
  } else {
    const FighterClass = _FIGHTER_CLASS_MAP[def.type] || Fighter;
    fighter = new FighterClass(def);
  }
  fighter.fighterIndex = fighterIndex;
  return fighter;
}

// Load the map after a microtask to ensure customFighters.js has finished loading
Promise.resolve().then(() => _loadFighterClassMap());

// Helper to get projectiles from the projectile system
export function getProjectiles() {
  return projectileSystem ? projectileSystem.getProjectiles() : [];
}
...
// Uses a dedicated offscreen canvas for flame particles
// to reduce draw calls and improve FPS
// ─────────────────────────────────────────────
...
/**
 * Initialize the flame canvas with the same dimensions as the main canvas
 */
...
/**
 * Resize the flame canvas to match the main canvas
 */
...
/**
 * Get the flame canvas and context
 */
...
/**
 * Draw all flame projectiles to the flame canvas as a unified cone/fan shape
 * Groups flames by owner fighter and draws a single expanding fan per fighter
 * The cone starts narrow at the origin and widens as it extends outward
...
export function drawFlamesToCanvas(flames) {
  ...
  // NOTE: We now skip rendering physical flame projectiles here because 
  // Flame Warden has a dedicated high-performance visual particle system (flamewardenFlameSystem).
  // Drawing gradients for every physical projectile was causing massive FPS drops.
  return;
}

/**
 * Composite the flame canvas onto the main canvas
 * Call this once at the end of the render loop
 */
export function compositeFlameCanvas() {
  if (!_flameCanvasInitialized || !_flameCanvas) return;
  
  // Only draw if flame canvas has content (check if any pixels are non-zero)
  // For simplicity, we just draw it - the clearRect ensures it's empty when no flames
