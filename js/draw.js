...
import { drawShurikenProjectile, drawGraySwordProjectile, drawPoisonBottleCore } from './weaponVisuals.js';
import { drawGunSlingerBullet, drawGunSlingerMuzzleFlash } from './weaponGraphic/gunSlingerWeaponGraphics.js';
import { drawBomberExplosionGraphic, drawBomberGrenade, drawGrenadeTrail, drawBomberC4 } from './weaponGraphic/bomberWeaponGraphics.js';
...
import { initFlameCanvas, resizeFlameCanvas, drawFlamesToCanvas, clearFlameCanvas } from './state.js';
import { drawDeathEffects } from './deathShatterEffect.js';
import { drawBloodEffects } from './bloodEffect.js';
import { drawIllusionDeathEffects } from './illusionDeathEffect.js';
import { drawIllusionSpawnEffects } from './illusionSpawnEffect.js';
import { drawBerserkerRageEffects } from './berserkerRageEffect.js';
export { drawDeathEffects, drawBloodEffects, drawIllusionDeathEffects, drawIllusionSpawnEffects, drawBerserkerRageEffects };
...
export function drawCronosSphereVisual({
  ...
  const cacheKey = `sphere_${Math.round(R)}`;
  let gridData = _cronosGridCache.get(cacheKey);

  if (!gridData) {
    const cellSize = Math.max(20, R * 0.14);
...
function drawCrimsonSniperBullet(ctx, x, y, angle, r, history) {
  ctx.save();
  ...
  if (history && history.length > 0) {
    ...
    // Thick Crimson Glow
    ctx.strokeStyle = 'rgba(255, 20, 20, 0.4)';
    ctx.lineWidth = Math.max(10, r * 3.5);
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ctx.stroke();

    // Medium Orange-Red Glow
    ctx.strokeStyle = 'rgba(255, 80, 40, 0.7)';
    ctx.lineWidth = Math.max(5, r * 1.8);
    ctx.stroke();
  ...
  const headLength = r * 5.5; // Elongated satisfying laser tip
  ...
  ctx.beginPath();
  ctx.moveTo(headLength * 0.85, 0);
  ctx.lineTo(0, r * 0.6);
  ctx.lineTo(-headLength * 0.25, 0);
  ctx.lineTo(0, -r * 0.6);
  ctx.closePath();
  ctx.fill();
...
export function drawProjectiles() {
  ...
  projectiles.forEach((p) => {
    ...
    // Crimson Sniper bullet visual - intense piercing laser
    if (p.visual === 'crimsonSniperBullet') {
      const angle = Math.atan2(p.vy, p.vx);
      drawCrimsonSniperBullet(ctx, p.x, p.y, angle, p.r, p.history);
      return;
    }

    // Crimson Sniper wall hit flash
    if (p.visual === 'crimsonImpactFlash') {
      const lifeRatio = p.life / p.maxLife;
      ...
      grad.addColorStop(0.2, `rgba(255, 100, 100, ${lifeRatio * 0.8})`);
    ...
    // Crimson Sniper wall hit sparks
    if (p.visual === 'crimsonSpark') {
      const lifeRatio = Math.max(0, p.life / p.maxLife);
      ctx.globalAlpha = lifeRatio;
      ctx.globalCompositeOperation = 'lighter';
      ctx.fillStyle = Math.random() > 0.4 ? '#ffffff' : '#ff3333';
      ctx.strokeStyle = ctx.fillStyle;
      
      // Speed-based streak length
      const speed = Math.sqrt(p.vx * p.vx + p.vy * p.vy);
      const streakLength = Math.max(2, speed * 1.5);
      
      // Streak
      ctx.beginPath();
      ctx.moveTo(p.x, p.y);
      ctx.lineTo(p.x - (p.vx / speed) * streakLength, p.y - (p.vy / speed) * streakLength);
      ctx.lineWidth = p.r;
      ctx.lineCap = 'round';
      ctx.stroke();
...
export function drawFuelPickups() {
  ...
  fuelPickups.forEach(pickup => {
    ...
    // No pulsing effect, static radius
    ...
    // ── Outer glow ──
    const glowGrad = ctx.createRadialGradient(nx, ny, r * 0.6, nx, ny, r * 2.2);
    glowGrad.addColorStop(0, 'rgba(255, 150, 0, 0.5)');
    glowGrad.addColorStop(0.5, 'rgba(255, 80, 0, 0.25)');
    ...
    // ── Jerrycan dimensions ──
    const cw = r * 1.1;   // half width
    const ch = r * 1.4;   // half height
    const cr = r * 0.25;  // corner radius

    // ── Handle (Top Center, drawn behind body) ──
    const hw = cw * 1.0;
    const hh = r * 0.4;
    ctx.strokeStyle = '#c61616';
    ctx.lineWidth = r * 0.35;
    ctx.lineJoin = 'round';
    ctx.beginPath();
    ctx.moveTo(nx - hw / 2, ny - ch + r * 0.2);
    ctx.lineTo(nx - hw / 2, ny - ch - hh);
    ctx.lineTo(nx + hw / 2, ny - ch - hh);
    ctx.lineTo(nx + hw / 2, ny - ch + r * 0.2);
    ctx.stroke();

    // ── Spout (Top Left, drawn behind body) ──
    ctx.fillStyle = '#222';
    ctx.beginPath();
    const sx = nx - cw * 0.6;
    const sy = ny - ch;
    ctx.moveTo(sx - r * 0.2, sy);
    ...
    ctx.beginPath();
    ctx.ellipse(sx - r * 0.25, sy - r * 0.65, r * 0.3, r * 0.15, -Math.PI / 6, 0, Math.PI * 2);
    ctx.fill();

    // ── Main Body ──
    const bodyGrad = ctx.createLinearGradient(nx - cw, ny - ch, nx + cw, ny + ch);
    bodyGrad.addColorStop(0, '#ff4b1f'); // vibrant orange-red
    bodyGrad.addColorStop(1, '#c61616'); // deep red

    ctx.fillStyle = bodyGrad;
    ctx.beginPath();
    roundedRect(ctx, nx - cw, ny - ch, cw * 2, ch * 2, cr);
    ctx.fill();
    ...
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    roundedRect(ctx, nx - cw + 1.5, ny - ch + 1.5, cw * 2 - 3, ch * 2 - 3, cr - 1);
    ctx.stroke();

    // ── Jerrycan details (X shape indent) ──
    ctx.strokeStyle = 'rgba(0, 0, 0, 0.25)';
    ctx.lineWidth = r * 0.15;
    ctx.beginPath();
    const ix = cw * 0.6; // indent x half width
    const iy = ch * 0.6; // indent y half height
    // Diagonal 1
    ctx.moveTo(nx - ix, ny - iy);
    ctx.lineTo(nx + ix, ny + iy);
    // Diagonal 2
    ctx.moveTo(nx + ix, ny - iy);
    ctx.lineTo(nx - ix, ny + iy);
    // Border for the indent
    roundedRect(ctx, nx - ix, ny - iy, ix * 2, iy * 2, cr * 0.5);
    ctx.stroke();
