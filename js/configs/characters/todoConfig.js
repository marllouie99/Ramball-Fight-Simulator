...
export const todoConfig = {
    ...
    punchDamage: 7,               // Base damage per melee punch strike
    knockback: 6,                 // Moderate knockback impulse applied on hit
    punchSpeed: 22,               // Animation frames for a smooth punch
    punchRange: 55,               // Additional melee reach distance for reliable basic attacks
    basicPunchCooldown: 24,       // Cooldown in frames between basic punches while waiting for skill sequence (~0.40s)
    ...
    blackFlashDamageMultiplier: 3.0, // 2.0x damage multiplier on Black Flash punch after Boogie Woogie swap (7 * 2.0 = 14)
    ...
    evadeBuffDurationFrames: 50,  // Duration in frames (~0.83s) where Todo and teammate gain evasion buff after swap
    evadeChance: 0.40,            // 40% chance to completely evade/miss incoming enemy basic attacks
    ...
    rockDamage: 6,                // Impact damage if rock directly hits target
    rockLife: 240,                // Lifetime in frames that rock continues bouncing in arena (4.0 seconds)

    // Rock Proximity Counter-Attack Sequence
    rockCounterCooldown: 600,     // Cooldown in frames (10.0s at 60fps) between Rock Proximity Counter-Attack triggers
    ...
    rockCounterComboInterval: 14, // Frames between combo punches during the attack sequence
    rockArrivalPushback: 6.0,     // Initial physics hit pushback applied upon rock proximity arrival
    rockArrivalScreenShake: 0,    // Arena screen shake on rock proximity arrival
    rockCounterComboPushback: 3.5, // Intermediate punch physics hit pushback per strike during sequence
    comboPunchScreenShake: 0,     // Arena screen shake on intermediate combo flurry punches
    rockCounterFinisherPushback: 18.0, // Final finisher punch physics hit pushback launching enemy back
    ...
    enableUltimateOverlay: true,  // Toggle to enable/disable Todo ultimate arena PNG overlay
    ultimateOverlayPath: 'Assets/Overlays/Todo-ultimate-overlay.png',
    ultimateOverlayAlpha: 0.65,   // Semi-transparent overlay alpha (0.65) so arena floor & battle action show underneath
    ultimateOverlayZoom: 1.0,     // Zoom scale multiplier for the arena PNG overlay image
    ultimateOverlayOffsetX: 0,    // Horizontal position offset in arena
    ultimateOverlayOffsetY: 0,    // Vertical position offset in arena
    takadaDamageMultiplier: 0.0,  // 1.5x damage output multiplier on all punches & attacks (7 * 1.5 = 10.5)
    takadaSpeedMultiplier: 1.15,  // 1.15x movement speed boost multiplier
    takadaPunchCooldownMult: 1.50, // Fast punches during ultimate (24 * 0.65 = ~15-16 frames between punches)
    takadaClapCooldownMult: 0.6,  // 40% reduced cooldown for Skill 1 (Boogie Woogie) during ultimate (~1.2s vs 2.0s)
    ...
    enableTakadaBackgroundSong: true,                               // Toggle to enable/disable Takada idol background music during ultimate (plays in 1v1 matches)
    ...
    takadaBackgroundSongVolume: 0.50,                              // Background song volume
