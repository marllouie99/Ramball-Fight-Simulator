// ─────────────────────────────────────────────
// TACTICAL FORCE — AWP SKIN (Recon Sniper)
// Unified Tactical Operator: Recon sniper hood, comms headset, ballistic plate carrier, rangefinder monocular aperture.
// ─────────────────────────────────────────────

export function drawAwpSkin(ctx, fighter) {
  const r = (fighter && fighter.r) ? fighter.r : 25;
  const themeColor = (fighter && fighter.color) ? fighter.color : '#ef4444';
  ...
  // Dual .338 Lapua Heavy Magazine Chest Pouches
  for (let i = -1; i <= 0; i++) {
    const px = i * 11 + 1;
    const py = 5;
  ...
  // 3. Recon Sniper Ghillie Hood (-Y Upper Half)
  ...
  // Ghillie Fabric Foliage Strands
  ctx.fillStyle = midShade;
  ctx.beginPath();
  ctx.moveTo(-10, -r + 2);
  ctx.lineTo(-6, -r - 3);
...
export const drawSniperSkin = drawAwpSkin;
