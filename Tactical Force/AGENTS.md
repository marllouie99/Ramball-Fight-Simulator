...
## 3. Dynamic Map Obstacle Physics & Collision Standards
- **Dynamic Active Map Resolution**: NEVER hardcode `STARTER_MAP.obstacles` in physics loops (`physics.js`), projectile interception (`projectileSystem.js`), AI line-of-sight checks (`hasLineOfSight()`), or raycasting (`raycastToObstacles()`).
- **Standard Dynamic Query Pattern**:
  ```javascript
  ...
  if (activeObstacles && activeObstacles.length > 0) {
    // Perform collision, interception, or LOS checks against activeObstacles
  }
  ```
...
- **Universal Multi-Map Architecture**: All current and future tactical battlegrounds (Sector 01 Starter Facility, Sector 02 Monolith, etc.) must define standard `arena`, `obstacles`, and `spawns` definitions so collision, AI raycasting, and rendering execute seamlessly without map-specific hardcoded branching.
...
- **Ultra-Simple Minimalist Layout**:
  - Top HUD (Team 1 CT): Side-by-side cards.
  - Bottom HUD (Team 2 T): Side-by-side cards.
  - No additional colors: ONLY the operative's name and health bar use the fighter's theme color.
  - Minimal elements: Operative name on the left; clean plain-text stats (`DMG <damageStat>  <cur>/<max>  <hp>`) in subtle neutral color on the right; with a simple 5px solid health bar underneath.
  - Zero bloated badges, zero pill boxes, zero drop-shadow blur filters, zero nested sub-bars.

---

## 5. Tactical Map Geometry & Arena Dark Mode Standards
- **Clean Minimalist Geometry**: Solid slate walls (`#1e293b` fill, `#475569` border) and solid dark arena floor (`#0d1117` / `#0b0f19`).
...
## 8. Tactical Operative Hand Scaling & Kinetic Stance Standards
- **Unified Hand Scaling (FOC Standard)**: All Tactical Force operative skins and firearm renderers MUST render hands with a standard **`7.5px` radius** and **`lineWidth: 1.8px`** (matching FOC brawler and weapon hand scales).
- **Rule 20 Compliance**: ALWAYS check `!this.hideFrontHand` and `!state.showSkinOnly` before drawing tactical hands.
- **Dual-Hand Kinetic Firearm Alignment**:
  - **Pistol (Ghost)**: Two-handed combat stance (`handX: r * 0.75 + 2`, `handY: 5`), smoothly offsetting for reload magazine insertions.
  - **Carbine (Assault Operator)**: Primary hand at pistol grip (`r * 0.85 - 2, 7`), forward support hand securely gripping handguard shroud (`r * 0.85 + 24, 2`).
  - **Shotgun (Heavy SWAT Breacher)**: Trigger hand on receiver grip (`r * 0.85 - 4, 7`), pump forend hand smoothly tracking cycling pump action (`pumpTimer`).
  - **Sniper (Marksman)**: Trigger hand on stock grip (`r * 0.55 + 6, 6`), forward support hand cycling bolt handle and swapping magazine.

---

## 9. Tactical Game Mode Configuration Standards
...
- **Standardized Modes**:
  - `Tactical 2v2`: 2v2 CT (M4A1, SPAS-12) vs T (Desert Eagle, AWP), 3 rounds, 500 HP, Sector 01/02 maps.
